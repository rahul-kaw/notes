...
time would be 0, the second query of duration 1 would have to wait 5 seconds (the duration of the first query) to be executed, 
...
def minimumWaitingTime(queries):
    ...
    for i in range(1,len(queries)):
        ...
        queries[i-1] = waitTime
    return sum(queries[:len(queries)-1])
...
waitTime = 0
queries.sort() # in place sort of array
for id,duration in enumerate(queries):
    queriesLeft = len(queries) - (id+1)
